...
//custom stage object
Declare_Any_Class( "Stage_p1", 	//front and back of the stage
  { 'populate'()                  // vector, and texture-space coordinate.
      { this.positions      = [ vec3(0,0,0), vec3(10,0,0), vec3(8,-2,0), vec3(2,-2,0)];  
        this.normals        = [ vec3(0,0,1), vec3(0,0,1), vec3(0,0,1), vec3(0,0,1) ];   // ...
        this.texture_coords = [ vec2(0,.5),   vec2(.25,.5),   vec2(.25,.375), 	vec2(0,.375)  ];   // ...
        this.indices        = [ 0, 1, 2,	2,3,0 ];                                 
      }
  }, Shape )
  
  Declare_Any_Class( "Stage_p2",  //left and right sides
  { 'populate'()                  // vector, and texture-space coordinate.
      { this.positions      = [ vec3(0,0,0), vec3(4,0,0), vec3(2,-2,0) ];  
        this.normals        = [ vec3(0,0,1), vec3(0,0,1), vec3(0,0,1)];   // ...
        this.texture_coords = [ vec2(0,.375),   vec2(.125,.375),   vec2(.0625,.25)];   // ...
        this.indices        = [ 0, 1, 2];                                 
      }
  }, Shape )
  
   Declare_Any_Class( "Stage_p3",  //top
  { 'populate'()                  // vector, and texture-space coordinate.
      { this.positions      = [ vec3(0,0,0), vec3(0,4,0), vec3(10,4,0), vec3(10,0,0)];  
        this.normals        = [ vec3(0,0,1), vec3(0,0,1), vec3(0,0,1), 	vec3(0,0,1) ];   // ...
        this.texture_coords = [ vec2(0,.5),   vec2(0,1),   vec2(1,1), 	vec2(1,.5)   ];   // ...
        this.indices        = [ 0, 1, 2,	2,3,0 ];                                 
      }
  }, Shape )
  
  Declare_Any_Class( "Stage",  
  { populate()                  
      {     var m_transform= rotation(60, vec3(1,0,0));
			Stage_p1.prototype.insert_transformed_copy_into( this, [], m_transform );
			
			m_transform= identity();
			m_transform= rotation(-90, vec3(1,0,0));
			Stage_p3.prototype.insert_transformed_copy_into( this, [], m_transform );
			
			m_transform= identity();
			m_transform= mult(rotation(-90, vec3(0,1,0)), rotation(60, vec3(1,0,0)));
			m_transform= mult(m_transform, translation(-4,0,0));
			Stage_p2.prototype.insert_transformed_copy_into( this, [], m_transform );
			
			m_transform= identity();
			m_transform= mult(m_transform, translation(0,0,-4));
			m_transform= mult(m_transform, scale(1,1,-1));
			m_transform= mult(m_transform, rotation(60, vec3(1,0,0)));
			Stage_p1.prototype.insert_transformed_copy_into( this, [], m_transform );
			
			m_transform= identity();
			m_transform= mult(m_transform, translation(10,0,0));
			m_transform= mult(m_transform, rotation(90, vec3(0,1,0)));
			m_transform= mult(m_transform, rotation(60, vec3(1,0,0)));
			Stage_p2.prototype.insert_transformed_copy_into( this, [], m_transform );			
      }
  }, Shape )

  Declare_Any_Class( "CustomRec", 
  { 'populate'()     
      {                    
        this.positions     .push( vec3(-1,0,0), vec3(1,0,0), vec3(-1,1,0), vec3(1,1,0) );
        this.normals       .push( vec3(0,0,1), vec3(0,0,1), vec3(0,0,1), vec3(0,0,1) );     // ...
        this.texture_coords.push( vec2(0,.5),   vec2(1,.5),   vec2(0,1),   vec2(1,1)   );     // ...
        this.indices       .push( 0, 1, 2,     1, 3, 2 );
	  }
  }, Shape )
	  
  Declare_Any_Class( "ThunderStrip", 
  { 'populate'()     
      {                    
        this.positions     .push( vec3(-1,0,0), vec3(1,0,0), vec3(-1,4,0), vec3(1,4,0) );
        this.normals       .push( vec3(0,0,1), vec3(0,0,1), vec3(0,0,1), vec3(0,0,1) );     // ...
