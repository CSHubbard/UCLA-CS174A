...
Declare_Any_Class( "Bee_Scene",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  { 'construct'( context )
      { var shapes = { "box" : new        Cube(), 
        ...
        this.define_data_members( { yellow_clay: context.shaders_in_use["Phong_Model"].material( Color(.94,.79,  0, 1 ), .2, 1, .7, 40 ),
                                     brown_clay: context.shaders_in_use["Phong_Model"].material( Color(.32,.19,.09, 1 ), .2, 1,  1, 40 ),
									 black_clay: context.shaders_in_use["Phong_Model"].material( Color(  0,  0,  0, 1 ), .2, 1,  1, 40 ),
									grass_green: context.shaders_in_use["Phong_Model"].material( Color(  0,.48,.05, 1 ), .2, 1,  1, 40 ),
									  bee_brown: context.shaders_in_use["Phong_Model"].material( Color(.60,.36,.06, 1 ), .2, 1,  1, 40 ),
									wing_silver: context.shaders_in_use["Phong_Model"].material( Color(.83,.83,.83, 1 ), .2, 1,  1, 40 ) } );
      },
	...
	'draw_Ground'( model_transform, graphics_state)
      { 
		...
		this.shapes.box.draw(graphics_state, model_transform, this.grass_green );	
      },
	  ...
	  'draw_Tree'(model_transform, graphics_state)
	  {
		  ...
		  {
			model_transform = mult( model_transform, translation(0, 2, 0) );
			model_transform = mult( model_transform, rotation(Math.sin(t)*5, 0, 0, 1));
			model_transform = mult( model_transform, translation(0, 2, 0) );
		  ...
		  model_transform = mult( model_transform, translation(0, 12, 0) );
		  model_transform = mult( model_transform, scale(10, 10, 10) );
		  this.shapes.ball.draw(graphics_state, model_transform, this.grass_green );
	  },
	  
	  'draw_Leg'(model_transform, graphics_state)
	  {
		  var t = graphics_state.animation_time/1000;
		  var angleAmt = Math.cos(t*2.5+2.35)
		  
		  if (angleAmt < 0)
			  angleAmt = -1*angleAmt;
		  
		  //leg segment one
		  model_transform = mult(model_transform, rotation(angleAmt*20+90, 1, 0, 0));
		  model_transform = mult(model_transform, translation(0, .5, 1));
		  model_transform = mult(model_transform, scale( .5, .5, 1));
		  this.shapes.box.draw( graphics_state, model_transform, this.bee_brown );
		  model_transform = mult(model_transform, scale( 2, 2, 1));
		  
		  //leg segment two
		  model_transform = mult(model_transform, translation(0, 0, 1))
		  model_transform = mult(model_transform, rotation(angleAmt*20, 1, 0, 0));
		  model_transform = mult(model_transform, translation(0, 0, 1));
		  model_transform = mult(model_transform, scale( .5, .5, 1));
		  this.shapes.box.draw( graphics_state, model_transform, this.bee_brown );
	  ...
	  'draw_Wing'(model_transform, graphics_state)
	  {
		  var t = graphics_state.animation_time/1000;
		  model_transform = mult(model_transform, rotation(Math.sin(t*5)*60, 1, 0, 0));
		  model_transform = mult(model_transform, translation(0, .1, 5));
		  model_transform = mult(model_transform, scale(2, .1, 5));
		  this.shapes.box.draw( graphics_state, model_transform, this.wing_silver);
	  },
	  ...
	  'draw_Bee'(model_transform, graphics_state)
	  {
		  ...
		  //move into pos
		  model_transform = mult( model_transform, rotation( t*25, 0, 1, 0 ));
		  model_transform = mult( model_transform, translation( 10, Math.sin(t*5)*5+25, 30));
		  
		  var centerBee = model_transform;
		  ...
		  //draw head
		  model_transform = centerBee;
		  model_transform = mult( model_transform, translation(5, 0 , 0));
		  model_transform = mult( model_transform, scale(2, 2, 2));
		  this.shapes.ball.draw( graphics_state, model_transform, this.bee_brown );
		  
		  //draw legs
		  //starboard set
		  model_transform = centerBee;
		  model_transform = mult(model_transform,translation( 0, -2, 2));
		  this.draw_Leg( model_transform, graphics_state );
		  ...
		  model_transform = centerBee;
		  model_transform = mult(model_transform,translation( 0, 2, 2));
		  this.draw_Wing( model_transform, graphics_state );
    ...
    'display'( graphics_state )
      { // Assignment 1 can start here!
		...
		customRot = rotation( 50*t, [          .1,          .8,             .1 ] );
		
        graphics_state.lights = [ new Light( mult_vec( customRot, vec4(  30,  30,  34, 1 ) ), Color( Math.sin(t)*.98, Math.cos(t)*.84, Math.cos(t)*.65, 1 ), Math.cos(t/2)*100000),
                                  new Light( mult_vec( customRot, vec4( -10, -20, -14, 0 ) ), Color( Math.cos(t)*.66, Math.sin(t)*.39, Math.sin(t)*.78, 1 ), Math.sin(t/2)*100000 )];
								  
		this.draw_Ground	(model_transform, graphics_state);
		this.draw_Tree		(model_transform, graphics_state);
